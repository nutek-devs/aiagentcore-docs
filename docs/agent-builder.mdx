...
For the full mechanics — entry routing, the `handoff_to_agent` built-in tool, Shared Conversation Context (SCC), agent-path tracking and depth limits — see [Multi-Agent Routing & Handoff](/multi-agent-routing).

## See also

- [Getting Started](/) — account setup and API token
- [Authentication](/authentication) — API auth flow
- [Multi-Agent Routing & Handoff](/multi-agent-routing) — compose multiple agents
