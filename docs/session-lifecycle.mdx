...
When you use Chat2Desk and configure inbound status webhooks (see [Webhooks](/webhooks)), VigilSquad auto-closes or marks sessions based on what the human operator does on the Chat2Desk side:
...
There is no extra configuration needed on the VigilSquad side — the webhook just has to be registered for the integration. See [Webhooks → Register-webhook helper](/webhooks) for the one-click registration flow.
