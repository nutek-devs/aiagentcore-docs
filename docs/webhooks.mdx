...
VigilSquad receives inbound messages from messaging platforms via webhooks. When a customer sends a message on WhatsApp, Telegram, or any connected channel, the platform delivers it to your webhook endpoint for processing.
...
1. Create a Chat2Desk integration in the VigilSquad Dashboard
...
Chat2Desk also delivers **status events** to the same webhook URL when a human operator acts on a dialog. VigilSquad consumes these events to keep the AI session in sync — for example automatically closing the session when the operator closes the ticket.
...
| `hook_type` | Internal action | Effect on VigilSquad session |
|---|---|---|
...
No extra configuration is needed on the VigilSquad side — the webhook just has to be registered (see **Register-webhook helper** below).
...
VigilSquad verifies the authenticity of incoming webhooks using **HMAC-SHA256 signatures**.
...
3. VigilSquad recomputes the hash and compares it using timing-safe comparison
...
You don't have to copy and paste the webhook URL into the Chat2Desk dashboard by hand — VigilSquad can register (or re-register) the webhook on your behalf.
...
When a webhook is received, VigilSquad processes it asynchronously:
...
Some platforms send a GET request to verify the webhook URL. VigilSquad handles this automatically:
