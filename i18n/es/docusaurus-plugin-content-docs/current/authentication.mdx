...
VigilSquad utiliza **autenticación mediante Bearer token**. Todos los endpoints protegidos de la API requieren un token válido en el encabezado `Authorization`.
...
1. Inicia sesión en [app.vigilsquad.com](https://app.vigilsquad.com)
...
```bash
curl -X POST https://api.vigilsquad.com/api/auth/register \
...
```bash
curl -X POST https://api.vigilsquad.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "your-secure-password"
...
```bash
curl -X POST https://api.vigilsquad.com/api/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{
    "refresh_token": "v1.MjE0..."
  }'
...
curl -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  https://api.vigilsquad.com/api/auth/me
```
