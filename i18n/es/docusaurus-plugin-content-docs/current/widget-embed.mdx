...
El widget de VigilSquad es una interfaz de chat liviana que incorporas en tu sitio web. Se conecta a tu agente de IA a través de WebSocket y soporta respuestas en streaming en tiempo real.
...
```html
<script
  src="https://api.vigilsquad.com/widget/vigilsquad-widget.js"
...
```html
<script src="https://api.vigilsquad.com/widget/vigilsquad-widget.js"></script>
<script>
  VigilSquad.init({
    integrationId: 42,
    host: "https://api.vigilsquad.com",
    lang: "en"
  });
</script>
```
...
| `removeBranding` | `boolean` | `false` | Ocultar "Powered by VigilSquad" |
...
- **Persistencia de sesión:** ID del cliente almacenado en `localStorage` (`vigilsquad_cid_{integrationId}`)
...
```javascript
const widget = VigilSquad.init({ integrationId: 42 });
