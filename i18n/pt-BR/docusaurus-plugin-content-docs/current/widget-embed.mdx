...
O Widget do VigilSquad e uma interface de chat leve que voce incorpora no seu site. Ele se conecta ao seu agente de IA via WebSocket e suporta respostas em streaming em tempo real.
...
- **Persistencia de sessao:** ID do cliente armazenado no `localStorage` (`vigilsquad_cid_{integrationId}`)
